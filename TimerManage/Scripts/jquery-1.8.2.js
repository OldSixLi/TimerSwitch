...
var
	...
	navigator = window.navigator, 	// Map over jQuery in case of overwrite
	...
	core_trim = String.prototype.trim, 	// Define a local copy of jQuery
	...
	core_rnotwhite = /\S/,
	core_rspace = /\s+/, 	// Make sure we trim BOM and NBSP (here's looking at you, Safari 5.0 and IE)
	...
	jquery: "1.8.2", 	// The default length of a jQuery object is 0
...
jQuery.support = (function() { 	var support,
		all,
		a,
		select,
		opt,
	...
	div.innerHTML = "  <link/><table></table><a href='/a'>a</a><input type='checkbox'/>"; 	all = div.getElementsByTagName("*");
	...
	support.radioValue = input.value === "t"; 	input.setAttribute( "checked", "checked" ); 	// #11217 - WebKit loses check when the name is after the checked attribute
	...
	support.appendChecked = input.checked; 	fragment.removeChild( input );
	...
	jQuery(function() {
		...
		if ( window.getComputedStyle ) {
			...
			support.shrinkWrapBlocks = ( div.offsetWidth !== 3 ); 			container.style.zoom = 1;
		} 		// Null elements to avoid leaks in IE
		body.removeChild( container );
		container = div = tds = marginDiv = null;
	}); 	// Null elements to avoid leaks in IE
	fragment.removeChild( div );
	all = a = select = opt = input = fragment = div = null; 	return support;
})();
var rbrace = /(?:\{[\s\S]*\}|\[[\s\S]*\])$/,
	rmultiDash = /([A-Z])/g; jQuery.extend({
	cache: {}, 	deletedIds: [], 	// Remove at next major release (1.9/2.0)
	...
	data: function( key, value ) {
		...
		part = parts[1] + "!"; 		return jQuery.access( this, function( value ) { 			if ( value === undefined ) {
			...
			this.each(function() {
				var self = jQuery( this ); 				self.triggerHandler( "setData" + part, parts );
...
var nodeHook, boolHook, fixSpecified,
	...
	getSetAttribute = jQuery.support.getSetAttribute; jQuery.fn.extend({
	attr: function( name, value ) {
		...
		if ( (value && typeof value === "string") || value === undefined ) {
			removes = ( value || "" ).split( core_rspace ); 			for ( i = 0, l = this.length; i < l; i++ ) {
				elem = this[ i ];
				if ( elem.nodeType === 1 && elem.className ) { 					className = (" " + elem.className + " ").replace( rclass, " " ); 					// loop over each item in the removal list
	...
	attrFn: {}, 	attr: function( elem, name, value, pass ) {
		...
		var propName, attrNames, name, isBool,
			i = 0; 		if ( value && elem.nodeType === 1 ) { 			attrNames = value.split( core_rspace ); 			for ( ; i < attrNames.length; i++ ) {
...
if ( !getSetAttribute ) { 	fixSpecified = {
		name: true,
		id: true,
		coords: true
 ...
 */
jQuery.event = { 	add: function( elem, types, handler, data, selector ) { 		var elemData, eventHandle, events,
			t, tns, type, namespaces, handleObj,
			handleObjIn, handlers, special; 		// Don't attach events to noData or text/comment nodes (allow plain objects tho)
	...
	remove: function( elem, types, handler, selector, mappedTypes ) { 		var t, tns, type, origType, namespaces, origCount,
			j, events, special, eventType, handleObj,
			elemData = jQuery.hasData( elem ) && jQuery._data( elem ); 		if ( !elemData || !(events = elemData.events) ) {
			return;
		...
		for ( t = 0; t < types.length; t++ ) {
			...
			for ( j = 0; j < eventType.length; j++ ) {
				handleObj = eventType[ j ]; 				if ( ( mappedTypes || origType === handleObj.origType ) &&
	...
	customEvent: {
		...
		event = typeof event === "object" ?
			...
			new jQuery.Event( type ); 		event.type = type;
		...
		if ( !onlyHandlers && !event.isDefaultPrevented() ) { 			if ( (!special._default || special._default.apply( elem.ownerDocument, data ) === false) &&
				...
				if ( ontype && elem[ type ] && ((type !== "focus" && type !== "blur") || event.target.offsetWidth !== 0) && !jQuery.isWindow( elem ) ) { 					// Don't re-trigger an onFOO event when we call its FOO() method
					...
					elem[ type ]();
					jQuery.event.triggered = undefined; 					if ( old ) {
						elem[ ontype ] = old;
					}
				}
			}
		...
		event = jQuery.event.fix( event || window.event ); 		var i, j, cur, ret, selMatch, matched, matches, handleObj, sel, related,
		...
		for ( i = 0; i < handlerQueue.length && !event.isPropagationStopped(); i++ ) {
			...
			event.currentTarget = matched.elem; 			for ( j = 0; j < matched.matches.length && !event.isImmediatePropagationStopped(); j++ ) {
				...
				if ( run_all || (!event.namespace && !handleObj.namespace) || event.namespace_re && event.namespace_re.test( handleObj.namespace ) ) { 					event.data = handleObj.data;
					event.handleObj = handleObj; 					ret = ( (jQuery.event.special[ handleObj.origType ] || {}).handle || handleObj.handler )
	...
	props: "attrChange attrName relatedNode srcElement altKey bubbles cancelable ctrlKey currentTarget eventPhase metaKey relatedTarget shiftKey target timeStamp view which".split(" "), 	fixHooks: {}, 	keyHooks: {
		...
		var i, prop,
			...
			copy = fixHook.props ? this.props.concat( fixHook.props ) : this.props; 		event = jQuery.Event( originalEvent ); 		for ( i = copy.length; i; ) {
			prop = copy[ --i ];
...
jQuery.event.handle = jQuery.event.dispatch; jQuery.removeEvent = document.removeEventListener ?
	...
	function( elem, type, handle ) {
		var name = "on" + type; 		if ( elem.detachEvent ) { 			// #8545, #7054, preventing memory leaks for custom events in IE6-8 –
...
var runtil = /Until$/,
	...
	find: function( selector ) {
		var i, l, length, n, r, ret,
			...
			length = ret.length;
			jQuery.find( selector, this[i], ret ); 			if ( i > 0 ) {
...
function createSafeFragment( document ) {
	...
	fragmentDiv = safeFragment.appendChild( document.createElement("div") ); wrapMap.optgroup = wrapMap.option;
...
if ( !jQuery.support.htmlSerialize ) {
	...
	remove: function( selector, keepData ) {
		...
		deepDataAndEvents = deepDataAndEvents == null ? dataAndEvents : deepDataAndEvents; 		return this.map( function () {
	...
	}, 	domManip: function( args, table, callback ) { 		// Flatten any nested arrays
		args = [].concat.apply( [], args ); 		var results, first, fragment, iNoClone,
			i = 0,
			value = args[0],
			scripts = [],
	...
	if ( dest.mergeAttributes ) {
		...
		// a checked appearance if the defaultChecked value isn't also set 		dest.defaultChecked = dest.checked = src.checked; 		// IE6-7 get confused and end up setting the value of a cloned
	...
	if ( args.length === 1 && typeof first === "string" && first.length < 512 && context === document &&
		...
		(jQuery.support.html5Clone || !rnoshimcache.test( first )) ) { 		// Mark cacheable and look for a hit
		cacheable = true;
	...
	jQuery.fn[ name ] = function( selector ) {
		var elems,
			...
			l = insert.length,
			parent = this.length === 1 && this[0].parentNode; 		if ( (parent == null || parent && parent.nodeType === 11 && parent.childNodes.length === 1) && l === 1 ) {
...
function fixDefaultChecked( elem ) {
	...
	clone: function( elem, dataAndEvents, deepDataAndEvents ) {
		var srcElements,
			destElements,
			i,
			clone; 		if ( jQuery.support.html5Clone || jQuery.isXMLDoc(elem) || !rnoshimcache.test( "<" + elem.nodeName + ">" ) ) {
		...
		if ( dataAndEvents ) {
			cloneCopyEvent( elem, clone ); 			if ( deepDataAndEvents ) {
				srcElements = getAll( elem );
				destElements = getAll( clone ); 				for ( i = 0; srcElements[i]; ++i ) {
...
var curCSS, iframe, iframeDoc,
	...
	elemdisplay = {}, 	cssShow = { position: "absolute", visibility: "hidden", display: "block" },
	cssNormalTransform = {
		letterSpacing: 0,
		fontWeight: 400
	}, 	cssExpand = [ "Top", "Right", "Bottom", "Left" ],
	cssPrefixes = [ "Webkit", "O", "Moz", "ms" ], 	eventsToggle = jQuery.fn.toggle; // return a css property mapped to a potentially vendor prefixed property
...
if ( window.getComputedStyle ) {
	curCSS = function( elem, name ) {
		var ret, width, minWidth, maxWidth,
			...
			style = elem.style; 		if ( computed ) { 			ret = computed[ name ];
			...
			if ( rnumnonpx.test( ret ) && rmargin.test( name ) ) {
				...
				ret = computed.width; 				style.width = width;
	...
	curCSS = function( elem, name ) {
		...
		// http://erik.eae.net/archives/2007/07/27/18.54.15/#comment-102291 		// If we're not dealing with a regular pixel number
...
function css_defaultDisplay( nodeName ) {
	...
	if ( display === "none" || display === "" ) {
		...
		if ( !iframeDoc || !iframe.createElement ) {
			...
			iframeDoc.close();
		} 		elem = iframeDoc.body.appendChild( iframeDoc.createElement(nodeName) ); 		display = curCSS( elem, "display" );
	...
	jQuery.cssHooks.opacity = {
		get: function( elem, computed ) {
			...
			if ( value >= 1 && jQuery.trim( filter.replace( ralpha, "" ) ) === "" &&
				style.removeAttribute ) { 				// Setting style.filter to null, "" & " " still leave "filter:" in the cssText
...
var r20 = /%20/g,
	...
	rselectTextarea = /^(?:select|textarea)/i; jQuery.fn.extend({
	serialize: function() {
...
function addToPrefiltersOrTransports( structure ) { 	// dataTypeExpression is optional and defaults to "*"
	return function( dataTypeExpression, func ) { 		if ( typeof dataTypeExpression !== "string" ) {
			...
			i = 0,
			length = dataTypes.length; 		if ( jQuery.isFunction( func ) ) {
...
function inspectPrefiltersOrTransports( structure, options, originalOptions, jqXHR,
		dataType /* internal */, inspected /* internal */ ) { 	dataType = dataType || options.dataTypes[ 0 ];
	inspected = inspected || {}; 	inspected[ dataType ] = true; 	var selection,
		list = structure[ dataType ],
		i = 0,
		length = list ? list.length : 0,
		executeOnly = ( structure === prefilters ); 	for ( ; i < length && ( executeOnly || !selection ); i++ ) {
...
jQuery.each( "ajaxStart ajaxStop ajaxComplete ajaxError ajaxSuccess ajaxSend".split( " " ), function( i, o ){
	...
	lastModified: {},
	etag: {} }); /* Handles responses to an ajax request:
...
jQuery.ajaxPrefilter( "json jsonp", function( s, originalSettings, jqXHR ) { 	var callbackName, overwritten, responseContainer,
		...
		replaceInData = hasCallback && !replaceInUrl && typeof data === "string" &&
			...
			rjsonp.test( data ); 	// Handle iff the expected data type is "jsonp" or we have a parameter to set
...
jQuery.ajaxTransport( "script", function(s) { 	// This transport only deals with cross domain requests
	if ( s.crossDomain ) { 		var script,
			head = document.head || document.getElementsByTagName( "head" )[0] || document.documentElement; 		return { 			send: function( _, callback ) { 				script = document.createElement( "script" ); 				script.async = "async"; 				if ( s.scriptCharset ) {
...
if ( jQuery.support.ajax ) { 	jQuery.ajaxTransport(function( s ) {
		...
		if ( !s.crossDomain || jQuery.support.cors ) { 			var callback; 			return {
				send: function( headers, complete ) { 					// Get a new xhr
					...
					callback = function( _, isAbort ) { 						var status,
							statusText,
							responseHeaders,
							responses,
							xml; 						// Firefox throws exceptions when accessing properties
...
function createFxNow() {
	...
	jQuery.each( props, function( prop, value ) {
		...
		for ( ; index < length; index++ ) {
			if ( collection[ index ].call( animation, prop, value ) ) { 				// we're done with this property
				return;
			}
		}
	});
		...
		animation = deferred.promise({
			...
			stop: function( gotoEnd ) {
				var index = 0,
					...
					length = gotoEnd ? animation.tweens.length : 0; 				for ( ; index < length ; index++ ) {
	...
	if ( elem.nodeType === 1 && ( "height" in props || "width" in props ) ) {
		...
		if ( jQuery.css( elem, "display" ) === "inline" &&
				jQuery.css( elem, "float" ) === "none" ) { 			// inline-level elements accept inline-block;
...
// to setting things on disconnected nodes Tween.propHooks.scrollTop = Tween.propHooks.scrollLeft = {
	set: function( tween ) {
...
var rroot = /^(?:body|html)$/i; jQuery.fn.offset = function( options ) {
	...
	return {
		...
		offset       = this.offset(),
		parentOffset = rroot.test(offsetParent[0].nodeName) ? { top: 0, left: 0 } : offsetParent.offset(); 		// Subtract element margins
